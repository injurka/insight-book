...
# 3 Реализовать плагин (микрофронтенд сообщества)

Хочу сделать плагин который является переносом того что есть вот здесь @/home/injurka/my/dao-of-ink/apps/client/src/06.screens/scroll/* `<ScrollDesktop class="absolute inset-0 z-10" />`. Главное стили должны быть не unocss, а scss или обычный css.

Суть в том, что у меня не будет выбора свитков по желанию, а просто сразу будет даваться случайный свиток из тех слов что предоставит приложение из словаря слов которые нужно повторить. А далее у меня будет проверка, если есть это слово в списке `data/` возможных слов то оно берется и генерируется hex доска по краям которой будут ключи данного слова. Размер доски зависит от кол-во ключей, если 3 ключа, то радиус 3 иероглифа, если 4 то 4, но минимум всегда 3. Суть доски это соединить все иероглифы между собой чем то схожим что есть у обоих, у меня заранее есть все иероглифы и информация о них @/home/injurka/my/dao-of-ink/packages/data/src/tier/* просто скопируй в проект. По сути это некое изучение свитков которое было в моде ThaumCraft.
