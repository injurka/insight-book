...
*   🔤 **Поддержка сложных языков (NLP)**: Встроенные морфологические токенизаторы для правильного разбиения текста на слова:
    ...
    *   🇷🇺 **Русский** (`az`)
*   📊 **Анализ сложности книги и трекинг**: ИИ оценивает уровень сложности (например, HSK 4, B2) и собирает частотный словарь. А ваш ежедневный прогресс отображается на графике активности (Heatmap).
...
*   **ИИ интеграция:** AI SDK (совместимость с OpenAI API, работает с Gemini, ChatGPT, Ollama и др. для анализа, TTS и OCR).
...
# Опционально: API ключи для TTS / OCR, если они отличаются
TTS_API_KEY=
...
```bash
docker compose exec -it docker-insight-book-server-1 bun ./src/cli.ts list
```

**Добавить нового пользователя:**
```bash
docker compose exec -it docker-insight-book-server-1 bun ./src/cli.ts add <логин> <пароль>
```

**Сменить пароль (в том числе администратору):**
```bash
docker compose exec -it docker-insight-book-server-1 bun ./src/cli.ts passwd <логин> <новый_пароль>
```
...
docker compose exec -it docker-insight-book-server-1 bun run dump:create

# Восстановить БД и файлы из последнего дампа S3
docker compose exec -it docker-insight-book-server-1 bun run dump:seed
```
