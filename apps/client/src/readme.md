...
```text
src/
├── 00.plugins/          # Плагинная система (Microkernel, DI, i18n)
├── 01.shared/           # Ядро: сервисы, типы, константы, composables, глобальные сторы, workers
├── 02.kit/              # UI-кит (dumb-компоненты)
│   ├── atoms/           # Простейшие элементы (KitBtn, KitInput, KitSkeleton, KitImage)
│   ├── molecules/       # Составные элементы (KitTabs, KitSelect, KitDropdown, KitToast)
│   └── organisms/       # Сложные блоки (KitDialog, KitPrompt, KitActivityHeatmap, KitToastManager)
├── 03.domain/           # Бизнес-сущности и привязанный к ним UI (Book, Flashcard)
├── 04.features/         # Пользовательские сценарии (Авторизация, Анализ, Глобальные экшены)
├── 05.modules/          # Сложные модули / Виджеты (Reader, Dictionary, Library, Settings)
├── 06.layouts/          # Скелеты страниц (DefaultLayout)
└── 07.views/            # Маршрутизация (Vue Router views)
```

Нумерованные префиксы слоёв (`00`–`07`) явно задают направление зависимостей: слой может импортировать только слои с меньшим номером (например, `05.modules` → `04.features` → … → `00.plugins`, но не наоборот).
...
Домены стараются не зависеть друг от друга напрямую. Взаимодействие происходит на уровне шины событий (Event Bus) или композиции в `07.views`.
...
Приложение поддерживает расширение функционала "на лету" без изменения ядра. Для этого реализован `00.plugins/plugin-manager.ts`.
...
- **Lazy-Loading:** Выбранные пользователем плагины (сохраненные в `Pinia / localStorage`) подгружаются динамически, предотвращая загрузку лишнего кода (уменьшение бандла) для неактивных расширений.
...
- **Separation of Concerns (Разделение ответственности):** 
  - UI-компоненты в `02.kit` не знают о бизнес-логике.
  - API-сервисы (`01.shared/services/api.service.ts`) не знают о Pinia.
