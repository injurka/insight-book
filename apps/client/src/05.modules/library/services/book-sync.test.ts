...
describe('startWholeBookSync', () => {
  ...
  it('analyzes missing sentences via server cache check and batch analysis', async () => {
    ...
    hoisted.analysisRepo.analyzeBatch.mockImplementation(async (_id: number, items: { id: string | number }[]) => ({
      results: items.map((it: { id: string | number }) => ({ id: it.id, analysis: { meanings: ['llm'] } })),
    }))
